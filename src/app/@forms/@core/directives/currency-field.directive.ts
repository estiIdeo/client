...
export class CurrencyFormatterDirective implements OnDestroy {
  ...
  private static formatter: Intl.NumberFormat = new Intl.NumberFormat('he-IL', { maximumFractionDigits: 2, style: 'currency', currency: 'ILS' });
  ...
  ngAfterViewInit() {
    ...
    this.ngControl
      .control
      .valueChanges
      .pipe(takeUntil(this.destroy$), untilDestroyed(this), distinctUntilChanged(), debounceTime(50))
  ...
  updateValue = (value: string) => {
    ...
    this.setValue(!!inputVal ?
      CurrencyFormatterDirective.validateDecimalValue(inputVal?.toString()?.replace(/[^0-9.]/g, '')) : '');
  }

  @HostListener('focus') onFocus() {
    this.setValue(CurrencyFormatterDirective.unFormatValue(this.ngControl.value));
  }
  ...
  static formatPrice(v: string) {
    return CurrencyFormatterDirective.formatter.format(+(v?.toString()?.replace(/[^0-9.]/g, '')) || 0);
  }

  static unFormatValue(v: string) {
    return (Number.parseFloat(v?.toString()?.replace(/,/g, '').replace(/[^0-9.]/g, '')) || 0)?.toFixed(2);
  }
